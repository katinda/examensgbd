...
class FermetureRepository {
    ...
    public function findGlobales(): array { $stmt = $this->pdo->query("SELECT * FROM Fermetures WHERE Site_ID IS NULL"); return array_map(fn($row) => $this->hydrateOne($row), $stmt->fetchAll(PDO::FETCH_ASSOC)); }
